document.addEventListener('DOMContentLoaded', function() {
    ...
    // Place this code after the DOMContentLoaded listener in script.js
...
function startRecording() {
  navigator.mediaDevices.getUserMedia({ audio: true })
    .then(stream => {
      ...
      // Correctly toggle visibility of buttons
      document.getElementById('mic-icon').style.display = 'none';
      document.getElementById('stop-mic-icon').style.display = 'block';
    })
    .catch(err => {
      console.error("Error accessing the microphone", err);
      // Keep the start recording button visible on error
      document.getElementById('mic-icon').style.display = 'block';
      document.getElementById('stop-mic-icon').style.display = 'none';
    });
}
...
// Function to send audio to the server and update the search input with the transcription
function sendAudioToServer() {
  ...
  .then(transcription => {
    document.getElementById('city-search').value = transcription;
  })
  .catch(err => {
...
document.getElementById('mic-icon').addEventListener('click', startRecording);
document.getElementById('stop-mic-icon').addEventListener('click', stopRecording);
